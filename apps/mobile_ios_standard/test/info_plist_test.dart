...
void main() {
  group('Info.plist Configuration Tests for Mobile iOS Standard', () {
    ...
    test('Info-standard.plist contains UIBackgroundModes with audio and NSMicrophoneUsageDescription', () {
      final file = File('ios/Runner/Info-standard.plist');
      expect(file.existsSync(), isTrue, reason: 'ios/Runner/Info-standard.plist must exist');

      final content = file.readAsStringSync();

      expect(content.contains('<key>UIBackgroundModes</key>'), isTrue,
          reason: 'Info-standard.plist must define UIBackgroundModes');
      expect(content.contains('<string>audio</string>'), isTrue,
          reason: 'Info-standard.plist must include audio background mode');
      expect(content.contains('<key>NSMicrophoneUsageDescription</key>'), isTrue,
          reason: 'Info-standard.plist must define NSMicrophoneUsageDescription');
    });
  });
}
