                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 0) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(L10n.homeReceiveOn)
                                .font(.system(size: 13, weight: .semibold))
                                .foregroundColor(.white.opacity(0.7))
                            Text(L10n.webBaseURL)
                                .font(.system(size: 13, weight: .bold))
                                .foregroundColor(kPrimary)
                                .underline()
                                .onTapGesture { openURL(L10n.webFullURL) }
                        }
                    ...
                    VStack(spacing: 16) {
                        NavigationLink(destination: SourceSelectView()) {
                            actionCard(title: L10n.homeBroadcast, subtitle: L10n.homeBroadcastSub,
                                       icon: "dot.radiowaves.left.and.right", primary: true)
                        }
                        NavigationLink(destination: ReceiveView()) {
                            actionCard(title: L10n.homeReceive, subtitle: L10n.homeReceiveSub,
                    ...
                    VStack(spacing: 12) {
                        Text(L10n.homeEngine)
                            .font(.system(size: 10, weight: .bold))
                            .tracking(2)
                            .foregroundColor(.white.opacity(0.15))
                        HStack(spacing: 0) {
                            footerLink(L10n.homeTerms) { openURL("\(L10n.webFullURL)/terms.html") }
                            footerSeparator()
                            footerLink(L10n.homePrivacy) { openURL("\(L10n.webFullURL)/privacy.html") }
                            footerSeparator()
                            footerLink(L10n.homeHelp) { openURL("mailto:mingh.liu@gmail.com") }
                        }
                    }
                    .padding(.bottom, 8)
                }
