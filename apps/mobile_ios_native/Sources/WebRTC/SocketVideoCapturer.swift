...
final class SocketServer {
    ...
    private func parseBuffer() {
        while true {
            // 缓冲必须以 HTTP 头开头；垃圾数据重新同步到下一个 "HTTP/1.1"
            if buffer.first != UInt8(ascii: "H") {
                if let r = buffer.range(of: Data("HTTP/1.1".utf8)) {
                    buffer.removeSubrange(..<r.lowerBound)
                    continue
                }
                // 无完整头部候选：保留末尾 8 字节（可能是被截断的 "HTTP/1.1" 前缀），等待更多数据
                if buffer.count > 8 {
                    buffer.removeSubrange(..<(buffer.count - 8))
                }
                return
            }
            ...
            // 校验：合法帧长度 256B~8MB；非法则丢弃这个假头重新同步
            guard length > 256, length < 8_000_000, width > 0, height > 0 else {
                buffer.removeSubrange(..<range.upperBound)
                continue
            }
