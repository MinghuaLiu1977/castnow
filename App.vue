...
<template>
  <div
    class="min-h-[100dvh] flex flex-col bg-slate-950 text-slate-50 font-sans selection:bg-amber-500/30"
  >
    <header
      v-if="appState !== STATES.RECEIVER_ACTIVE"
    ...
    >
      ...
      >
        ...
        />
        <span
          class="bg-gradient-to-r from-slate-100 to-slate-400 bg-clip-text text-xl font-black italic uppercase tracking-tight text-transparent pr-1"
          >CastNow</span
        >
      ...
      <div class="flex items-center gap-2">
        <div
          class="px-3 py-1 bg-slate-900 rounded-full border border-slate-800 flex items-center gap-2"
        >
    ...
    <main class="flex-1 flex flex-col relative overflow-hidden">
      <Transition name="fade" mode="out-in">
        <!-- Landing -->
        <div
          v-if="appState === STATES.LANDING"
          class="flex-1 flex flex-col items-center justify-center p-6 text-center"
        >
          <div
            class="mb-4 px-4 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full flex items-center gap-2"
          >
            <Globe class="w-3 h-3 text-amber-500" />
            <span
              class="text-[10px] font-black uppercase text-amber-500 tracking-widest"
              >Secure P2P Protocol Engine</span
            >
          </div>
          <h1
            class="text-6xl md:text-8xl font-black mb-6 tracking-tighter leading-none"
          >
          ...
          <div class="flex flex-col gap-4 w-full max-w-xs">
            <button
              @click="appState = STATES.SOURCE_SELECT"
            ...
            <a
              href="https://minghster.gumroad.com/l/ihhtg"
              target="_blank"
            ...
            >
              ...
              <span
                class="text-xs font-black uppercase tracking-widest text-amber-500"
                >7-Day Unlimited Pass</span
              >
            </a>
        ...
        <!-- Source Selection -->
        <div
          v-else-if="appState === STATES.SOURCE_SELECT"
          class="flex-1 flex flex-col items-center justify-center p-6"
        >
          <h2
            class="text-2xl font-black uppercase mb-12 tracking-widest flex items-center gap-3"
          >
          ...
          <div class="grid grid-cols-1 gap-4 w-full max-w-sm">
            ...
            <button
              @click="handleStartCasting('screen')"
            ...
            >
              ...
              <div>
                <span class="block font-black uppercase tracking-widest text-lg"
                  >Screen Share</span
                >
            ...
            <button
              @click="handleStartCasting('camera')"
        ...
        <!-- Sender View -->
        <div
          v-else-if="appState === STATES.SENDER"
        ...
        >
          ...
          >
            ...
            <div class="mb-10">
              <p
                class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4"
              >
                Sharing Access Key
              </p>
              <div class="flex items-center justify-center gap-2">
                <template v-for="(char, i) in peerId.split('')" :key="i">
                  ...
                  <span
                    v-if="i === 2"
                    class="text-slate-800 font-black text-2xl"
                    >-</span
                  >
                </template>
                <div v-if="!peerId" class="flex gap-2">
                  <div
                    v-for="n in 6"
                    :key="n"
                    class="w-12 h-16 bg-slate-950 rounded-2xl border border-slate-800 animate-pulse"
                  ></div>
                </div>
              </div>
            </div>
        ...
        <!-- Receiver Input -->
        <div
          v-else-if="appState === STATES.RECEIVER_INPUT"
          class="flex-1 flex flex-col items-center justify-center p-6"
        >
          <h2
            class="text-2xl font-black uppercase mb-10 tracking-widest flex items-center gap-3"
          >
            <span class="w-8 h-[2px] bg-amber-500"></span>
            Enter Access Key
            <span class="w-8 h-[2px] bg-amber-500"></span>
          ...
          <div
            v-if="isTouchDevice"
            class="grid grid-cols-3 gap-4 w-full max-w-[280px] mb-8"
          >
            <button
              v-for="n in 9"
              :key="n"
          ...
          <div
            v-else
            class="mb-12 text-slate-500 text-xs font-bold uppercase tracking-widest animate-pulse"
          >
          ...
          <p
            v-if="error"
            class="mt-4 text-red-500 text-xs font-bold uppercase tracking-widest flex items-center gap-2"
          >
        ...
        <div
          v-else-if="appState === STATES.BROADCAST_ENDED"
          class="flex-1 flex items-center justify-center p-6 bg-slate-950"
        >
          <div
            class="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-[3rem] p-10 text-center shadow-2xl"
          >
            <div
            ...
            <h3
              class="text-3xl font-black uppercase mb-4 tracking-tight text-white"
            >
              Broadcast Ended
            </h3>
        ...
        <!-- Receiver Active -->
        <div
          v-else-if="appState === STATES.RECEIVER_ACTIVE"
        ...
        >
          ...
          <div
            class="absolute top-6 left-6 transition-opacity duration-300 z-10"
            :class="{ 'opacity-0': !showControls }"
          >
            <button
              @click="resetApp"
            ...
            <div>
              <div class="flex items-center gap-2 mb-2">
                ...
                <span
                  ...
                  >Live Feed</span
                >
                <div v-if="!isPro" class="ml-2 px-2 py-0.5 bg-slate-800/80 backdrop-blur-md rounded-md border border-slate-700 flex items-center gap-1.5 transition-opacity duration-300" :class="{'opacity-100': remainingSeconds < 300, 'opacity-0': remainingSeconds >= 300 && !showControls}">
                   <div :class="{'bg-red-500 animate-pulse': remainingSeconds < 300, 'bg-slate-400': remainingSeconds >= 300}" class="w-1.5 h-1.5 rounded-full"></div>
                   <span class="text-[9px] font-bold" :class="{'text-red-500': remainingSeconds < 300, 'text-slate-300': remainingSeconds >= 300}">
                     {{ remainingSeconds < 300 ? `Ending in ${Math.floor(remainingSeconds / 60)}:${(remainingSeconds % 60).toString().padStart(2, '0')}` : 'Free Session: 30m Limit' }}
                   </span>
                </div>
    ...
    <!-- Info Modal -->
    <div v-if="showInfo" class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl" @click.self="showInfo = null">
       <div class="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-[2.5rem] p-10 shadow-2xl relative">
          <button @click="showInfo = null" class="absolute top-8 right-8 text-slate-500 hover:text-white transition-colors">
             <X class="w-6 h-6" />
          </button>
          ...
          <div v-if="showInfo === 'source'">
             ...
             <p class="text-slate-400 text-sm leading-relaxed mb-8">
                CastNow is an open-source high-performance P2P engine. The source code is available on GitHub under the MIT license for community contributions and transparency.
             </p>
             <a href="https://github.com/MinghuaLiu1977/castnow" target="_blank" class="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 text-slate-950 font-black rounded-xl uppercase tracking-widest text-[10px] hover:scale-105 transition-transform active:scale-95">
                View on GitHub
             </a>
          </div>
          ...
          <div v-else-if="showInfo === 'privacy'">
             ...
             <button @click="showInfo = null" class="w-full py-4 bg-slate-800 text-white font-black rounded-xl uppercase tracking-widest text-[10px] active:scale-95 transition-all">Close</button>
          </div>
          ...
          <div v-else-if="showInfo === 'terms'">
             ...
             <p class="text-slate-500 text-[11px] italic mb-8 border-l-2 border-amber-500/20 pl-4">
                The service is provided 'as is' without warranties of any kind. Misuse of the platform for illegal activities will result in termination of service availability.
             </p>
             <button @click="showInfo = null" class="w-full py-4 bg-slate-800 text-white font-black rounded-xl uppercase tracking-widest text-[10px] active:scale-95 transition-all">Agree & Close</button>
          </div>
       </div>
    </div>
  </div>
